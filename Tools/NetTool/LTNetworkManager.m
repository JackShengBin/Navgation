...
+ (void)httpRequest:(NetRequestType)type
    ...
    NSMutableDictionary *dictM = [NSMutableDictionary dictionaryWithDictionary:para];
//    NSString *token = @"token";//[BSKeychain keyChainLoad];
//    if (token.length > 0 || [url isEqualToString:@""]) {
//        [dictM setValue:token forKey:@"token"];
//        [dictM setValue:NSStringFormat(@"%@|%@|%@|%@",machineName,systemName,CurrentSystemVersion,[OpenUDID value])  forKey:@"deviceSn"];
//    }
    if ([[LTGetCurrentVC lt_iphoneModel] isEqualToString:@"iPhone X"]) {
        [dictM setValue:@"44cd6a88-d802-4065-8db5-c7eae1115719" forKey:@"token"];
        [dictM setValue:@"iPhone 7 Plus|iOS|11.4.1|76664e67467ac90c841730efc9517b4590b1929a" forKey:@"deviceSn"];
    }
    if (type == RequestTypeGET) {
        [[LTHTTPClient shareInstance] GET:url parameters:dictM progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            success(responseObject);
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            failure(error);
        }];
    }else{
        [[LTHTTPClient shareInstance] POST:url parameters:dictM progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            success(responseObject);
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            failure(error);
        }];
